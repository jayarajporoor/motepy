	...
	else
	{
		...
		for(var i=0;i<ast.params.length;i++){
			...
			if(flow_param){
				...
				mpbuild.error("Flow param mismatch for ", next_state.name, ". flow from ", scoped_name);
			}
		}
	...
	if(typeof ast.up !== 'undefined'){
		...
		if(ast.qid && ast.qid.length > 1){
			...
			if(sym && !sym.info.is_temp){
				...
				if(!sym){
					mpbuild.warning("Symbol ", id , " not found");
				}
			}
		}
        if(typeof ast.dim != 'undefined'){
        	if(!sym){
        		mpbuild.error("The array symbol ", id, " not found in symbol table.");
        	}else{
...
function boiler_plate(){
	strglobals.push("/********************************************************************************");
	strglobals.push("This code is automatically generated by the MotePy compiler. DO NOT EDIT!");
	strglobals.push("********************************************************************************/");
}
